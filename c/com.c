...
struct _Com
{
	...
	int MPCInfoNum[STAGE_NUM];
	MPCInfo *MPCInfo[STAGE_NUM];
	int CurrentMPCInfoNum;
	MPCInfo *CurrentMPCInfo;
};
...
static int Com_MidSearchRoot(Com *self, int in_depth, int in_color, int in_opponent, int *out_move);
...
void Com_SetRandom(Com *self, int in_value, int in_ratio)
{
	self->Random = in_value * DISK_VALUE;
	self->RandomRatio = in_ratio;
}
...
int Com_NextMove(Com *self, const Board *in_board, int in_color, int *out_value)
{
	...
	if (result != NOMOVE) {
		...
		Evaluator_SetStage(self->Evaluator, 0);
		...
		Evaluator_SetStage(self->Evaluator, left - self->MidDepth);
		id = Evaluator_GetStageIndex(left - self->MidDepth);
		self->CurrentMPCInfoNum = self->MPCInfoNum[id];
		self->CurrentMPCInfo = self->MPCInfo[id];
		value = Com_MidSearchRoot(self, self->MidDepth, color, Board_OpponentColor(color), &result);
	}
	if (out_value) {
		*out_value = value;
	}
...
static int Com_OpeningSearch(Com *self, int in_color, int in_opponent, int *out_move)
{
	MoveList *p;
	PositionInfo info;
	PositionInfo base_info;
	int current_value;
	int value, max = -MAX_VALUE;
	int count = 0;
	const int bound = DISK_VALUE * 2;
	const int min_random = DISK_VALUE * 30;

	*out_move = NOMOVE;
	if (!self->UseOpening || !self->Opening) {
		return max;
	}
	if (Opening_Info(self->Opening, self->Board, in_color, &base_info)) {
		current_value = -base_info.Value;
	} else {
	...
	for (p = self->Moves->Next; p; p = p->Next) {
		if (Board_Flip(self->Board, in_color, p->Pos)) {
			if (Opening_Info(self->Opening, self->Board, in_opponent, &info)) {
				value = -info.Value;
                if (self->Random * self->RandomRatio >= min_random && value < current_value && 
                	value >= current_value - bound && value >= -bound) {
                	value += bound;
                }
				if (value > max) {
					*out_move = p->Pos;
...
static int Com_MidSearchRoot(Com *self, int in_depth, int in_color, int in_opponent, int *out_move)
{
	int value, max = -MAX_VALUE, second = -MAX_VALUE;
	int second_move;
	int move;
	...
	*out_move = info[0].Move->Pos;
	for (i = 0; i < info_num; i++) {
		...
		value = -Com_MidSearch(self, in_depth - 1, -MAX_VALUE, -max + self->Random + 1, in_opponent, in_color, 0, &move);
...
#endif
		if (value > max) {
			second_move = *out_move;
			second = max;
			*out_move = info[i].Move->Pos;
			max = value;
		} else if (value > second) {
			second_move = info[i].Move->Pos;
			second = value;
		}
	}
#if 1
	if (max - second <= self->Random && get_rand(100) < self->RandomRatio) {
		count = 0;
		for (i = 0; i < info_num; i++) {
			if (max > info[i].Value && max - info[i].Value < self->Random) {
				count++;
				if (count == 1 || get_rand(count) < 1) {
					*out_move = info[i].Move->Pos;
					max = info[i].Value;
				}
			}
...
#else
	if (max - second <= self->Random / 2 && get_rand(100) < self->RandomRatio ||
		max - second <= self->Random && get_rand(100) < self->RandomRatio / 2) {
		*out_move = second_move;
		max = second;
	}
#endif
	return max;
...
static int Com_MidSearch(Com *self, int in_depth, int in_alpha, int in_beta, int in_color, int in_opponent, int in_pass, int *out_move)
{
	...
	if (in_depth == 0) {
		...
		self->Node++;
		disk_count = Board_CountDisks(self->Board, in_color);
		op_disk_count = Board_CountDisks(self->Board, in_opponent);
		if (disk_count == 0) {
			return -DISK_VALUE * BOARD_SIZE * BOARD_SIZE;
		} else if (op_disk_count == 0) {
			return DISK_VALUE * BOARD_SIZE * BOARD_SIZE;
		} else {
	...
	if (in_depth >= MPC_DEPTH_MIN && in_depth < MPC_DEPTH_MIN + self->CurrentMPCInfoNum) {
		mpc_info = &self->CurrentMPCInfo[in_depth - MPC_DEPTH_MIN];
		value = max - mpc_info->Deviation + mpc_info->Offset;
	...
	*out_move = NOMOVE;
	if (in_depth > 2) {
		...
		if (*out_move != NOMOVE) {
			if (max > in_alpha) {
				hash_info.Upper = hash_info.Lower = max;
				hash_info.Move = *out_move;
			} else {
				hash_info.Upper = max;
				if (max > hash_info.Lower) {
					hash_info.Move = *out_move;
				}
			}
			Hash_Set(self->Hash, &hash_value, &hash_info);
	...
	if (!can_move) {
		if (in_pass) {
			...
			*out_move = NOMOVE;
			...
			op_disk_count = Board_CountDisks(self->Board, in_opponent);
			if (disk_count == 0) {
				max = -DISK_VALUE * BOARD_SIZE * BOARD_SIZE;
			} else if (op_disk_count == 0) {
				max = DISK_VALUE * BOARD_SIZE * BOARD_SIZE;
			} else {
	...
	*out_move = NOMOVE;
	if (in_depth > 8) {
		...
		if (*out_move != PASS && *out_move != NOMOVE) {
			if (max > in_alpha && max > hash_info.Lower) {
				hash_info.Upper = hash_info.Lower = max;
				hash_info.Move = *out_move;
			} else {
				hash_info.Upper = max;
...
static int Com_ReadMPCInfo(Com *self, FILE *fp)
{
	...
	for (i = 0; i < STAGE_NUM; i++) {
		if (fread(&self->MPCInfoNum[i], sizeof(int), 1, fp) < 1) {
			return 0;
		}
		if (self->MPCInfoNum[i] == 0) {
			if (self->MPCInfo[i]) {
				free(self->MPCInfo[i]);
			}
			self->MPCInfo[i] = NULL;
		...
		info = realloc(self->MPCInfo[i], sizeof(MPCInfo) * self->MPCInfoNum[i]);
		if (!info) {
			return 0;
		}
		self->MPCInfo[i] = info;
		if (fread(self->MPCInfo[i], sizeof(MPCInfo), self->MPCInfoNum[i], fp) < (size_t)self->MPCInfoNum[i]) {
			return 0;
		}
	}
	return 1;
